...
public class FritzConnection {
	...
	private static final Logger logger = LoggerFactory.getLogger(FritzConnection.class);
	...
	public void printInfo() {
		...
		for (String a : services.keySet()) {
			...
			for (String b : s.getActions().keySet()) {
				logger.info("    {}", b);
				logger.info("       {}", s.getActions().get(b).getArguments());
			}
		}
	}

	public String getSoapSecretString() {
		return getSoapSecretString(realm, serverNonce);
	}

	public String getSoapSecretString(String realm, String serverNonce) {

		MessageDigest md;
		try {
			md = MessageDigest.getInstance("MD5");
			String concat = user + ":" + realm + ":" + pwd;
			md.update(concat.getBytes());
			byte[] digest = md.digest();

			return getMD5(DatatypeConverter.printHexBinary(digest).toLowerCase() + ":" + serverNonce);
		} catch (NoSuchAlgorithmException e) {
			logger.error("got NoSuchAlgorithmException", e);
			return "";
		}
	}
