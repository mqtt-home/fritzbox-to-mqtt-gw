...
public class Response {
	...
	public Response(SOAPMessage response, Map<String, Type> stateToType, Map<String, String> argumentState)
		...
		if (response.getSOAPHeader() != null) {
			try {
				tmp = response.getSOAPHeader().getChildNodes();
				for (int i = 1; i < tmp.getLength() && headerNodes == null; i++) {
					if (tmp.item(i).getNodeName().equals("#text")) {
						continue;
					}
					headerNodes = tmp.item(i).getChildNodes();
				}
			...
			for (int i = 1; i < headerNodes.getLength(); i++) {
				if (headerNodes.item(i).getNodeName().equals("#text")) {
					continue;
				}
				headerData.put(headerNodes.item(i).getNodeName(), headerNodes.item(i).getTextContent());
			}
		}
	...
	public String toString() {
		return "Response [response=" + response + ", stateToType=" + stateToType + ", argumentState=" + argumentState
				+ ", data=" + data + "]";
	}
